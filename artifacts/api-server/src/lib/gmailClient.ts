...
async function ensureMigrated(
  ...
  if (!email) return;
  await db
    .insert(gmailAccountsTable)
    .values({
      userId,
      email,
      accessToken: maybeEncrypt(user.googleAccessToken) ?? null,
      refreshToken:
        maybeEncrypt(user.googleRefreshToken) ?? user.googleRefreshToken,
    ...
    const [found] = await db
      ...
      .where(
        and(
          ...
          eq(gmailAccountsTable.email, accountEmail),
        ),
      )
      .limit(1);
  ...
  if (!accessToken || expiresAt < Date.now() + 60_000) {
    ...
    if (account.id !== -1) {
      ...
      await db
        ...
        .set({
          googleAccessToken:
            maybeEncrypt(credentials.access_token ?? null) ?? null,
...
export async function isGmailConnected(
  ...
  try {
    ...
    if (accounts.length > 0)
      return { connected: true, email: accounts[0].email };
...
export async function getConnectedGmailAccounts(
  userId: string,
): Promise<
  Array<{
    email: string;
...
export function decodeBody(part: any): {
  body: string;
  bodyType: "html" | "plain";
} {
