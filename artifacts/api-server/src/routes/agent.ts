...
import {
  AgentRunBody as AgentRunBodySchema,
  AgentSendBody as AgentSendBodySchema,
...
async function validateBrowseUrl(
  rawUrl: string,
...
const TOOLS: ChatCompletionTool[] = [
  {
    ...
    function: {
      name: "search_emails",
      description:
        "Search Gmail for emails matching a query. Returns list of matching threads with subject, sender, date, and snippet.",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description:
              "Gmail search query (e.g. 'from:john@example.com', 'subject:invoice', 'United flight confirmation')",
          },
          maxResults: {
            type: "number",
  ...
  },
  {
    ...
    function: {
      name: "send_email",
      description:
        "Compose and send an email. Always present the email content to the user for confirmation before calling this tool. Only call this when the user has already approved the content.",
      parameters: {
        type: "object",
        properties: {
          ...
          threadId: {
            type: "string",
            description: "Optional: thread ID to reply within",
          },
        },
      ...
      parameters: {
        ...
        properties: {
          ...
          start: {
            type: "string",
            description:
              "Start time in ISO 8601 format (e.g. 2025-01-15T10:00:00)",
          },
  ...
  },
  {
    ...
    function: {
      name: "search_web",
      description:
        "Search the web for a query and return top result URLs and titles. Use this when you need to find a website URL before browsing it.",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description:
              "The search query (e.g. 'United Airlines check in', 'Delta flight status')",
          },
        },
        required: ["query"],
      },
  ...
  },
  {
    ...
    function: {
      name: "browse_url",
      description:
        "Navigate a browser to a URL and return a readable version of the page content including visible text and interactive elements (buttons, links, inputs). Opens a new browser session.",
      parameters: {
        type: "object",
        properties: {
          url: {
  ...
  },
  {
    ...
    function: {
      name: "get_page_state",
      description:
        "Get the current state of the browser page — the current URL and a snapshot of visible text and interactive elements. Use this after clicking or typing to confirm what happened.",
      parameters: {
        type: "object",
        properties: {},
      },
  ...
  },
  {
    ...
    function: {
      name: "click_element",
      description:
        "Click an element on the current browser page by matching a human description to the element's text, label, or role.",
      parameters: {
        type: "object",
        properties: {
          description: {
            type: "string",
            description:
              "Human description of the element to click (e.g. 'Check In button', 'Continue link', 'Submit form button')",
          },
        },
  ...
  },
  {
    ...
    function: {
      name: "type_text",
      description:
        "Locate a form field on the current browser page by description and type text into it.",
      parameters: {
        type: "object",
        properties: {
          field_description: {
            type: "string",
            description:
              "Human description of the form field (e.g. 'Last name field', 'Confirmation number input', 'Email address')",
          },
          text: {
            type: "string",
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "Search term to find files by name",
          },
        },
        required: ["query"],
      },
  ...
  },
  {
    ...
    function: {
      name: "drive_read",
      description:
        "Read the content of a Google Drive file (works best for Google Docs, Sheets, Slides, and text files).",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "List the user's recent Microsoft Teams chats and direct message conversations.",
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Send a direct message in a Microsoft Teams chat. Always confirm the message content with the user before sending.",
      parameters: {
        type: "object",
        properties: {
          chatId: {
            type: "string",
            description: "The Teams chat ID to send the message to",
          },
          content: {
            type: "string",
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "List the Microsoft Teams teams and groups the user belongs to.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Post a message to a Microsoft Teams channel. Always confirm the message content with the user before posting.",
      parameters: {
        type: "object",
        properties: {
          teamId: { type: "string", description: "The Teams team ID" },
          ...
          subject: {
            type: "string",
            description: "Optional subject/headline for the channel post",
          },
        },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Reply to a specific message thread in a Microsoft Teams channel.",
      parameters: {
        type: "object",
        properties: {
          teamId: { type: "string", description: "The Teams team ID" },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Create a Microsoft Teams online meeting. Always confirm details with the user first.",
      parameters: {
        type: "object",
        properties: {
...
async function executeListCalendarEvents(
  ...
  if (!events.length)
    return `No calendar events found in the next ${args.days || 7} days.`;
  return events
    .map((e) => {
...
async function cleanupSession(sessionId: string) {
  ...
  if (session) {
    try {
      ...
      logger.warn({ err, sessionId }, "[agent] failed to close session");
    }
...
async function runAgentCore(
  ...
  const sessionId =
    incomingSessionId && activeSessions.has(incomingSessionId)
      ? incomingSessionId
      : `${userId}-${randomUUID()}`;
    ...
    while (iteration < MAX_ITERATIONS) {
      ...
      if (onToken) {
        ...
        if (message.tool_calls?.length) {
          iterToolCalls = message.tool_calls.map((tc) => ({
            id: tc.id,
            type: "function" as const,
      ...
      for (const toolCall of iterToolCalls) {
        ...
        if (isBrowserTool && browserStepCount >= MAX_BROWSER_STEPS) {
          ...
          try {
            if (toolName === "search_emails") {
              toolOutput = await executeSearchEmails(
                userId,
                args as { query: string; maxResults?: number },
              );
            } else if (toolName === "read_email") {
              toolOutput = await executeReadEmail(
                userId,
                args as { threadId: string },
              );
              ...
              toolOutput =
                "Email queued for user confirmation. Present the email content to the user and ask them to confirm sending.";
            } else if (toolName === "list_calendar_events") {
              toolOutput = await executeListCalendarEvents(
                userId,
                args as { days?: number },
              );
            } else if (toolName === "create_calendar_event") {
              const calArgs = args as {
                title: string;
                start: string;
                end: string;
                description?: string;
              ...
              pendingCalendarEvent = {
                ...
                ...(calArgs.description
                  ? { description: calArgs.description }
                  : {}),
                ...(calArgs.location ? { location: calArgs.location } : {}),
                ...(calArgs.attendees?.length
                  ? { attendees: calArgs.attendees }
                  : {}),
              };
              toolOutput =
                "Calendar event queued for user confirmation. Present the event details to the user and ask them to confirm creating it.";
              ...
              if (!urlCheck.ok) {
                ...
                await session.page.goto(urlCheck.url.href, {
                  waitUntil: "domcontentloaded",
                  timeout: 20000,
                });
                const finalUrl = session.page.url();
                if (!isUrlSafe(finalUrl)) {
                  // Cleanup navigation to about:blank - ignore errors as this is non-critical
              ...
              if (!session) {
                toolOutput =
                  "No active browser session. Use browse_url to open a page first.";
                status = "error";
              } else {
                toolOutput = await getPageSnapshot(session.page);
              ...
              if (!session) {
                ...
                if (!rawDesc || !rawDesc.trim()) {
                  toolOutput =
                    "click_element requires a non-empty description.";
                  status = "error";
                } else {
                  const ROLE_WORDS =
                    /\b(button|link|the|a|an|click|on|press|tap|submit|form|element|item|option)\b/gi;
                  ...
                  const locators = [
                    session.page.getByRole("button", {
                      name: new RegExp(escapeRegex(normalizedDesc), "i"),
                    }),
                    session.page.getByRole("link", {
                      name: new RegExp(escapeRegex(normalizedDesc), "i"),
                    }),
                    session.page.getByRole("button", { name: descRegex }),
                    session.page.getByRole("link", { name: descRegex }),
                    session.page.getByText(
                      new RegExp(escapeRegex(normalizedDesc), "i"),
                    ),
                    session.page.getByText(descRegex),
                    session.page.locator(
                      `[aria-label*="${escapeCssAttrValue(normalizedDesc)}" i]`,
                    ),
                    session.page.locator(
                      `[title*="${escapeCssAttrValue(normalizedDesc)}" i]`,
                    ),
                    session.page.locator(
                      `[value*="${escapeCssAttrValue(normalizedDesc)}" i]`,
                    ),
                  ];
                  ...
                  if (clicked) {
                    // Wait for page load after click - ignore timeout errors as content may still be accessible
                    await session.page
              ...
              if (!session) {
                ...
                if (!fieldDesc || !fieldDesc.trim()) {
                  toolOutput =
                    "type_text requires a non-empty field_description.";
                  status = "error";
                } else {
                  const FIELD_ROLE_WORDS =
                    /\b(field|input|box|form|entry|area|the|a|an|text|enter|type|fill)\b/gi;
                  const normalizedField =
                    fieldDesc
                      .replace(FIELD_ROLE_WORDS, "")
                      .replace(/\s+/g, " ")
                      .trim() || fieldDesc.trim();
                  ...
                  const fieldLocators = [
                    session.page.getByLabel(
                      new RegExp(escapeRegex(normalizedField), "i"),
                    ),
                    session.page.getByLabel(fieldRegex),
                    session.page.getByPlaceholder(
                      new RegExp(escapeRegex(normalizedField), "i"),
                    ),
                    session.page.getByPlaceholder(fieldRegex),
                    session.page.locator(
                      `input[name*="${escapeCssAttrValue(fieldKey)}"]`,
                    ),
                    session.page.locator(
                      `input[id*="${escapeCssAttrValue(fieldKey)}"]`,
                    ),
                    session.page.locator(
                      `input[aria-label*="${escapeCssAttrValue(normalizedField)}" i]`,
                    ),
                    session.page.locator(
                      `textarea[name*="${escapeCssAttrValue(fieldKey)}"]`,
                    ),
                    session.page.locator(
                      `textarea[aria-label*="${escapeCssAttrValue(normalizedField)}" i]`,
                    ),
                  ];
              ...
              const r = await fetch(
                `https://${domain}/api/drive/search?q=${encodeURIComponent(a.query)}`,
                {
                  headers: {
              ...
              if (!r.ok) {
                toolOutput =
                  "Google Drive search failed. Make sure Google is connected.";
              } else {
                ...
                toolOutput = files.length
                  ? files
                      .map(
                        (f) =>
                          `ID: ${f.id} | Name: ${f.name} | Type: ${f.mimeType} | Modified: ${f.modifiedTime} | URL: ${f.webViewLink}`,
                      )
                      .join("\n")
              ...
              const domain =
                process.env.REPLIT_DOMAINS?.split(",")[0] || "localhost";
              const r = await fetch(
                `https://${domain}/api/drive/list?pageSize=${a.pageSize ?? 20}`,
                {
                  headers: {
                    Cookie: req.headers.cookie ?? "",
                    Authorization: req.headers.authorization ?? "",
              ...
              if (!r.ok) {
                toolOutput =
                  "Could not list Drive files. Make sure Google is connected.";
              } else {
                const data = (await r.json()) as {
                  files?: Array<{
                    id: string;
              ...
              const domain =
                process.env.REPLIT_DOMAINS?.split(",")[0] || "localhost";
              const r = await fetch(
                `https://${domain}/api/drive/file/${encodeURIComponent(a.fileId)}`,
                {
                  headers: {
                    Cookie: req.headers.cookie ?? "",
                    Authorization: req.headers.authorization ?? "",
              ...
              if (!r.ok) {
                ...
                const f = data.file;
                toolOutput = f
                  ? `File: ${f.name}\nType: ${f.mimeType}\nModified: ${f.modifiedTime}\nURL: ${f.webViewLink}\n${data.text ? `\nContent:\n${data.text}` : "(binary file — content not readable as text)"}`
                  : "File not found.";
              }
              ...
              if (!token) {
                toolOutput =
                  "Microsoft Teams is not connected. Ask the user to connect Teams in Settings → Connectors.";
              } else {
                const data = await teamsGet<{
                  value: Array<{
                    id: string;
                    chatType: string;
                    topic?: string;
                ...
                }>(token, "/me/chats?$expand=members&$top=30");
              ...
              if (!token) {
                ...
                await teamsPost(token, `/me/chats/${a.chatId}/messages`, {
                  body: { contentType: "text", content: a.content },
                });
              ...
              if (!token) {
                ...
                }>(token, "/me/joinedTeams?$select=id,displayName,description");
                const teams = (data.value || []).map(
                  (t) =>
                    `ID: ${t.id} | Name: ${t.displayName}${t.description ? ` | ${t.description}` : ""}`,
                );
              ...
              if (!token) {
                ...
                const data = await teamsGet<{
                  ...
                  token,
                  `/teams/${a.teamId}/channels?$select=id,displayName,description`,
                );
                const channels = (data.value || []).map(
                  (c) =>
                    `ID: ${c.id} | Name: ${c.displayName}${c.description ? ` | ${c.description}` : ""}`,
                );
                toolOutput = channels.length
                  ? channels.join("\n")
                  : "No channels found.";
              }
              ...
              if (!token) {
                ...
                await teamsPost(
                  token,
                  `/teams/${a.teamId}/channels/${a.channelId}/messages`,
                  body,
                );
              ...
              if (!token) {
                ...
                await teamsPost(
                  token,
                  `/teams/${a.teamId}/channels/${a.channelId}/messages/${a.messageId}/replies`,
                  { body: { contentType: "text", content: a.content } },
                );
              ...
              if (!token) {
                ...
                const meeting = await teamsPost<{
                  joinWebUrl?: string;
                  subject?: string;
                  id?: string;
                }>(token, "/me/onlineMeetings", {
                  subject: a.subject,
        ...
        if (isBrowserTool && status !== "error") {
          ...
          if (liveSession) {
            try {
              const buf = await liveSession.page.screenshot({
                type: "jpeg",
                quality: 55,
                fullPage: false,
              });
      ...
      if (pendingEmail) {
        ...
        if (!("content" in lastMsg) || lastMsg.role !== "assistant") {
          if (onToken) {
            ...
            if (!finalAnswer) {
              finalAnswer =
                "I've drafted an email for you. Please review and confirm.";
              onToken(finalAnswer);
            }
          } else {
            ...
            finalAnswer =
              pendingCompletion.choices[0]?.message?.content ||
              "I've drafted an email for you. Please review and confirm.";
          }
        } else {
          finalAnswer =
            "I've drafted an email for you. Please review and confirm sending it.";
...
function wrapAgentError(err: unknown): {
  status: number;
  ...
  return {
    status: 500,
    body: { error: "Agent task failed. Please try again." },
  };
}
...
async function saveAgentConversation(
  ...
  await db
    ...
    .values(newMessages.map((m) => ({ ...m, conversationId: conv.id })));
  return conv.id;
}
  ...
  try {
    ...
    if (reqConversationId) {
      const [conv] = await db
        ...
        .where(
          and(
            eq(agentConversations.id, reqConversationId),
    ...
    if (result.pendingEmail)
      send({ type: "pending_email", data: result.pendingEmail });
    if (result.pendingCalendarEvent)
      send({ type: "pending_event", data: result.pendingCalendarEvent });

    const savedConversationId = await saveAgentConversation(
      userId,
      task,
      result.answer,
      result.steps,
...
router.post("/agent/run", requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      ...(result.pendingCalendarEvent
        ? { pendingCalendarEvent: result.pendingCalendarEvent }
        : {}),
      ...(result.browserWasUsed
        ? { sessionId: result.sessionId, browserSessionActive: true }
        : {}),
    });
  } catch (err: unknown) {
  ...
  try {
    ...
    runAgentCore(
      ...
      .then((result) => {
        ...
        if (result.pendingCalendarEvent)
          job.pendingCalendarEvent = result.pendingCalendarEvent;
...
router.post("/agent/send", requireAuth, async (req, res) => {
  try {
    ...
    if (!sendParsed.success) {
      res.status(400).json({
        error: sendParsed.error.errors[0]?.message ?? "Invalid request body",
      });
      return;
    }
    ...
    const subjectLine =
      threadId && !subject.startsWith("Re:") ? `Re: ${subject}` : subject;
...
router.post("/agent/create-event", requireAuth, async (req, res) => {
  try {
    const userId = getReqUserId(req)!;
    const { title, start, end, description, location, attendees } =
      req.body as PendingCalendarEvent;
    if (!title || !start || !end) {
      res.status(400).json({ error: "title, start, and end are required" });
      return;
    }
    const message = await executeCreateCalendarEvent(userId, {
      title,
      start,
      end,
      description,
    ...
    const emailSummaries = await Promise.all(
      threads.slice(0, 12).map(async (t) => {
        try {
          ...
          const headers = lastMsg.payload?.headers || [];
          const from =
            parseEmailAddress(getHeader(headers, "From")).name ||
            parseEmailAddress(getHeader(headers, "From")).email;
          const subject = getHeader(headers, "Subject");
          const date = getHeader(headers, "Date");
